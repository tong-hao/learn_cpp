...
// This struct is used for sorting the values and keeping the owning process
// information intact.
typedef struct {
	int comm_rank;
	union {
...
// Gathers numbers for TMPI_Rank to process zero. Allocates enough space given
// the MPI datatype and returns a void * buffer to process 0. It returns NULL to
// all other processes.
void *gather_numbers_to_root(void *number, MPI_Datatype datatype,
	...
	// Allocate an array on the root process of a size depending on the MPI
	// datatype being used.
	int datatype_size;
...
// This function sorts the gathered numbers on the root process and returns an
// array of ordered by the process's rank in its communicator. Note - this
// function is only executed on the root process.
int *get_ranks(void *gathered_numbers, int gathered_number_count,
               MPI_Datatype datatype) {
	int datatype_size;
	MPI_Type_size(datatype, &datatype_size);

	// Convert the gathered number array to an array of CommRankNumbers. This
	// allows us to sort the numbers and also keep the information of the
	// processes that own the numbers intact.
	...
	for (i = 0; i < gathered_number_count; i++) {
		...
		memcpy(&(comm_rank_numbers[i].number),
		       gathered_numbers + (i * datatype_size), datatype_size);
	}
	...
	// Now that the comm_rank_numbers are sorted, create an array of rank values
	// for each process. The ith element of this array contains the rank value
	// for the number sent by process i.
...
// Gets the rank of the recv_data, which is of type datatype. The rank is
// returned in send_data and is of type datatype.
int TMPI_Rank(void *send_data, void *recv_data, MPI_Datatype datatype,
              MPI_Comm comm) {
	// Check base cases first - Only support MPI_INT and MPI_FLOAT for this
	// function.
	...
	// To calculate the rank, we must gather the numbers to one process, sort
	// the numbers, and then scatter the resulting rank values. Start by
	// gathering the numbers on process 0 of comm.
